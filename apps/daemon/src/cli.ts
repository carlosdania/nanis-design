...
// external agents (hermes-agent, openclaw, etc.) can drive Nanis Design
...
function printRootHelp() {
  ...
  od plugin open-design-pr <folder>
      Push a community-catalog branch and open the Nanis Design PR form.
  ...
  od share <open-design|url> [options]
      Build localized social-share targets for the Nanis Design repo or a
  ...
  od mcp [--daemon-url <url>]
      ...
      running Nanis Design daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local Nanis Design project and create
...
Options:
  --daemon-url <url>   Nanis Design daemon HTTP base.
...
Runs Tavily-backed shallow research through the local Nanis Design daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The Nanis Design daemon itself is unaffected - it can be ' +
...
tool calls to a running Nanis Design daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local Nanis Design
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   Nanis Design daemon HTTP base URL. Resolution
...
Tools exposed:
  list_projects                  list every Nanis Design project
...
user has open in Nanis Design; get_artifact and get_file additionally
...
→ MCP server in the Nanis Design app. The daemon must be running locally
...
Register Nanis Design's stdio MCP server into a coding agent's own config.

Agents:
  ${AGENT_SLUGS.join(' ')}

Options:
  --uninstall, --remove   Remove the Nanis Design MCP server instead.
...
Wraps GitHub CLI auth for Nanis Design registry publishing. The token stays in gh.`);
    return;
  }
...
Shows the GitHub account gh will use for Nanis Design registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
...
Common options:
  --daemon-url <url>   Nanis Design daemon HTTP base (default OD_DAEMON_URL, OD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not Nanis Design.`);
...
// these so a code agent can drive Nanis Design end-to-end without
...
Imports a local project directory as an editable Nanis Design design system.
...
Imports a public GitHub repository as an editable Nanis Design design system.
...
Imports a shadcn registry item as an Nanis Design design system.
