...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  queueDrain?: boolean;
  /** The Nanis Design Cloud balance gate already ran for this exact send at
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'Nanis Design selected or inferred these workspace contexts for this turn. Treat absolute paths as reference context unless the user explicitly asks to edit them.',
...
/** Set by the home create flow when its submit already ran the Nanis Design
...
export function ProjectView({
  ...
  const handleSend = useCallback(
    async (
      ...
      // Nanis Design Cloud pre-run balance gate: a definitively insufficient
  ...
  // "Share to Nanis Design" — kicks off the bundled `od-share-to-community`
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'Nanis Design PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the Nanis Design fork exists',
